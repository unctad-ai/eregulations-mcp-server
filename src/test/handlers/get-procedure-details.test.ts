...
describe('GetProcedureDetailsHandler', () => {
  ...
  it('has the correct name and description', () => {
    ...
    expect(handler.description).toContain('Get detailed information about a specific procedure');
  ...
  it('calls the API with the correct procedure ID and formats the result', async () => {
    ...
    // Verify logging
    expect(logger.log).toHaveBeenCalledWith(expect.stringContaining(`request for procedure ID ${mockProcedureId}`));
    expect(logger.log).toHaveBeenCalledWith(expect.stringContaining(`Successfully retrieved details for procedure ID ${mockProcedureId}`));
  });
  ...
  it('handles API errors correctly', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining(`Error in GET_PROCEDURE_DETAILS handler for ID ${mockProcedureId}`),
      testError.message
    );
    ...
    expect(result.content[0].text).toContain("Error retrieving procedure details: Procedure not found");
    expect(result.content[0].text).toContain("Valid procedure IDs can be found by using the listProcedures tool");
  });
  ...
  it('handles missing procedureId parameter gracefully', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining(`Error in GET_PROCEDURE_DETAILS handler for ID undefined`),
      testError.message
    );
    
    // Verify the response contains a helpful error message
    expect(result.content[0].text).toContain("Error retrieving procedure details");
  });
  
  it('handles non-Error exceptions correctly', async () => {
    // Setup API to throw a non-Error value
    vi.mocked(mockApi.getProcedureById).mockRejectedValue('API connection failed');
    ...
    expect(result.content[0].text).toContain("Error retrieving procedure details: API connection failed");
  });
});