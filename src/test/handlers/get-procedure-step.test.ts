...
describe("GetProcedureStepHandler", () => {
  ...
  it("calls the API with the correct IDs and formats the result", async () => {
    ...
    expect(mockApi.getProcedureStep).toHaveBeenCalledWith(
      mockProcedureId,
      mockStepId
    );
  ...
  it("handles API errors correctly", async () => {
    ...
    expect(result.content[0].text).toContain(
      "Error retrieving step details: Step not found"
    );
  });
  ...
  it("handles non-Error exceptions correctly", async () => {
    ...
    vi.mocked(mockApi.getProcedureStep).mockRejectedValue(
    ...
    expect(result.content[0].text).toContain(
      "Error retrieving step details: API connection failed"
    );
  });
});
