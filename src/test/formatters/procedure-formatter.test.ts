...
describe('ProcedureFormatter', () => {
  ...
  const mockProcedure: ProcedureData = {
    ...
    explanatoryText: "Process for obtaining an import license for restricted goods",
    isOnline: true,
    data: {
      id: 123,
      name: "Import License Application",
  ...
  it('formats procedure with all fields present', () => {
    ...
    expect(result.text).toContain('PROCEDURE: Apply for Import License (ID:123)');
    expect(result.text).toContain('URL: https://example.com/import-license');
    expect(result.text).toContain('DESC: Process for obtaining an import license for restricted goods');
  ...
  it('optimizes long descriptions', () => {
    const longDescProcedure: ProcedureData = {
      id: 789,
      name: "Test Procedure",
      explanatoryText: "A".repeat(300) // Text longer than our limit
    };

    const result = formatter.format(longDescProcedure);
    
    const explanatoryText = longDescProcedure.explanatoryText || '';
    expect(result.text).toContain('...');
    expect(result.text.length).toBeLessThan(explanatoryText.length);
  });
});